...
\begin{itemize}
\item Like key-value stores and document stores, they scale out with heterogeneous data. The data can be distributed across a cluster, replicated, and efficiently retrieved. They are also compatible with MapReduce-like parallelism paradigms.
...
Now that we have a gas of cells available, we can begin to play with it. The first idea that comes to mind is how to retrieve a cell from the gas (point query).
...
Given a set of dimensions $\mathbb{D}$, a set of members $\mathbb{M}$, a set of concepts $\mathbb{C}$ and a set of values $\mathbb{S}$, a cell is given by:
...
An instance of the cellstore is any subset of the world hypercube.
...
In practice, there are two reasons why the finite support condition is satisfied: first, the number of dimensions it itself finite. Second, default members are stored as the absence of any member, and a dimension associated with a default member is simply omitted. Hence, cells occupy a reasonable size in memory even with a high or even infinite number of dimensions.
...
A hypercube must furthermore must fulfil the constraint of finite support, that is, all but a finite number of dimensions must be associated to the default member singleton. This is similar to the constraint of finite support for cells.

$$\{d\in\mathbb{D}, M_\mathcal{Q}(d)\neq\{\mathcal{D}(d)\}\} \text{ is finite}$$

Given a hypercube query, cells can be retrieved from a cell store instance I:

$$I_\mathcal{Q}\subset I=\{i \in I | c_i\in C_\mathcal{Q} \wedge \forall d\in \mathbb{D}, m_i\in M_\mathcal{Q}(d)\cup D_\mathcal{Q}(d)\}$$ 
...
$$\exists i_1, i_2 \in I, c_{i_1}=c_{i_2} \wedge \forall d \in \mathbb{D}, m{i_1}(d)=m{i_2}(d)$$
...
This is joint work. The implementation of the first cell store on top of a document store has been made as a team effort by Matthias Brantner, William Candillon, Federico Cavalieri (also to be thanked for performing several rounds of proof reading), Dennis Knochenwefel, Alexander Kreutz and myself. We received a lot of very useful advice from Charles Hoffman.
