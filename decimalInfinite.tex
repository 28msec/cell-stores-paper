...
\begin{figure}
\caption{Natural encoding of an integer (base 2), not padded and padded}
...
\item the remaining digits (after the decimal point) are grouped 3 by 3 (declets). Each declet is encoded in its natural binary representation on 10 bits. Trailing 0s are added to make sure that the last group also has 3 digits.
\end{itemize}
...
If the obtained exponent is 0, but the exponent sign was encoded as negative (i.e., the overall decimalInfinite encoding begins with 01011 or 00100), an error is raised.
...
The encoding are designed in such a way that, if $a < b$, then the encoded $a$ ($STEM_a$) comes lexicographically before ($<<$) the encoded $b$ ($STEM_b$).

A proof thereof now follows.

$a$'s (absolute) exponent is called $e_a$, $b$'s exponent is called $e_b$. $a$'s significand is called $m_a$, b's significand is called $m_b$.
...
\item If $a$ and $b$ are both negative, then $S_a=S_b=01$.
  \begin{enumerate}
